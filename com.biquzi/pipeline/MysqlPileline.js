...
class MysqlPileline {
    ...
    asyncInsertNovel(item) {
        return new Promise((resolve, reject) => {
            let sql = `
                INSERT INTO dd_novels(name, author, intro, url, image_path, image_url, category_name, status, update_at, latest_chapter_name, latest_chapter_url)
                    SELECT ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? FROM DUAL
                    WHERE NOT EXISTS(SELECT id FROM dd_novels WHERE url=?)`;

            this.pool.query(sql, [item.name, item.author, item.intro, item.url, item.image_path, item.image_url, item.category_name, item.status, item.update_at, item.latest_chapter_name, item.latest_chapter_url, item.url],
                (err, results) => {
